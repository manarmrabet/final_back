...
public interface StockTransferRepository extends JpaRepository<StockTransfer, Long> {
    ...
    /**
     * FIX : ORDER BY nb DESC dans le JPQL — appelé avec PageRequest.of(0, 5) SANS sort
     * pour éviter l'erreur SQL Server "column specified more than once in order by".
     */
    @Query("""
    ...
    // ─── Recherche avancée ────────────────────────────────────────────────
    /**
     * FIX SQL Server : pas de ORDER BY dans la requête JPQL — géré par Pageable.
     * Utilise COALESCE pour les paramètres nullables.
     */
    @Query("""
        SELECT t FROM StockTransfer t
    ...
    // Dans StockTransferRepository.java
    @Query("""
    SELECT t FROM StockTransfer t
